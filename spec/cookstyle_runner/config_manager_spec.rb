  ...
  describe '.load_config' do
    context 'when required environment variables are missing' do
      it 'logs an error and exits if GITHUB_APP_ID is missing' do
        ...
        expect { ConfigManager.load_config(logger) }.to raise_error(SystemExit)
      end
    ...
    context 'when optional variables are set' do
      ...
      it 'returns the specified values for optional configuration' do
        ...
        expect(config[:pr_labels]).to eq(['label1', 'label2'])
        ...
        expect(config[:topics]).to eq(['topic-a', 'topic-b'])
        expect(config[:force_refresh_repos]).to eq(['repo1', 'repo2'])
        expect(config[:include_repos]).to eq(['repo3', 'repo4'])
      end
    end
  end
