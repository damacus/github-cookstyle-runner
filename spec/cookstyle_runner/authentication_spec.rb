...
RSpec.describe CookstyleRunner::Authentication do
  ...
  describe '.generate_jwt' do
    it 'generates a JWT with the correct payload' do
      ...
      jwt = CookstyleRunner::Authentication.send(:generate_jwt, app_id, private_key_pem)
    ...
    it 'reads private key from file path if path is given' do
      ...
      # Call with the path
      CookstyleRunner::Authentication.send(:generate_jwt, app_id, dummy_key_path)
    ...
    it 'uses private key directly if content is given' do
      ...
      # Call with content
      CookstyleRunner::Authentication.send(:generate_jwt, app_id, private_key_pem)
    end
  end
  ...
  describe '.get_installation_token' do
    it 'generates a JWT and uses it to request an installation token' do
      ...
      allow(CookstyleRunner::Authentication).to receive(:generate_jwt).with(app_id, dummy_key_path).and_return(fake_jwt)

      # Call get_installation_token with the path
      token = CookstyleRunner::Authentication.get_installation_token(app_id: app_id, installation_id: installation_id, private_key: dummy_key_path)

      expect(CookstyleRunner::Authentication).to have_received(:generate_jwt).with(app_id, dummy_key_path)
