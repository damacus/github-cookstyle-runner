...
RSpec.describe GitOperations do
  ...
  let(:logger) { instance_double(Logger, debug: nil, info: nil, warn: nil, error: nil, debug?: false) }
  let(:context) do
  ...
  describe '.clone_or_update_repo' do
    ...
    context 'when repo directory does not exist' do
      ...
      it 'clones the repository using the authenticated URL' do
        # The expected URL format is "https://ghs_faketoken123:x-oauth-basic@github.com/test-owner/test-repo.git"
        expect(Git).to receive(:clone).with(
          'https://ghs_faketoken123:x-oauth-basic@github.com/test-owner/test-repo.git', repo_dir
        ).and_return(mock_git_repo)
        expect(described_class).not_to receive(:update_repo)
        expect(mock_git_repo).to receive(:checkout).with(branch)

        result = described_class.clone_or_update_repo(context, branch)
    ...
    context 'on error' do
      it 'logs an error and exits if Authentication fails' do
        allow(CookstyleRunner::Authentication).to receive(:get_installation_token).and_raise(StandardError,
                                                                                             'Token fetch failed')
        expect(logger).to receive(:error).with(/Authentication failed/)
        expect do
          described_class.clone_or_update_repo(context, branch)
