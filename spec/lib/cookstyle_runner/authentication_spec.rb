...
RSpec.describe CookstyleRunner::Authentication do
  ...
  describe '.github_credentials' do
    ...
    it 'raises an error when no auth environment variables are set' do
      stub_const('ENV', {})
      expect { described_class.github_credentials }.to raise_error(RuntimeError, /No GitHub authentication available/)
    end
  end
