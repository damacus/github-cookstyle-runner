...
module CookstyleRunner
  ...
  class GitHubPRManager
    ...
    def create_pull_request(repository, base_branch, head_branch, title, body)
      ...
      max_retries = T.let(@settings.respond_to?(:retry_count) ? (@settings.retry_count || 3) : 3, Integer)
      ...
      begin
        RetryWithBackoff.with_exponential_backoff(
          ...
          context: { repo: repo_name, head_branch: head_branch, base_branch: base_branch }
        ) do
    ...
    def create_issue(repository, title, body)
      ...
      begin
        RetryWithBackoff.with_exponential_backoff(
          ...
          context: { repo: repo_name, title: title }
        ) do
